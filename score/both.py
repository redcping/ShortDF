...
def get_inception_and_fid_score(images, fid_cache, num_images=None,
    ...
    while True:
        ...
        with torch.no_grad():
            #batch_images=torch.nn.functional.interpolate(batch_images, size=(299, 299), mode='bilinear', align_corners=False)
    ...
    for i in range(splits):
        ...
        if use_torch:
            part = torch.clamp(part,min=1e-8) 
            kl = part * (
                torch.log(part) -
    ...
    print([t.shape for t in [m1,s1,m2,s2]])
